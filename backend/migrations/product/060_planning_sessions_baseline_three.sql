...
-- temporary name.
--
-- Completion plan P0: the confirmed planner rule is a three-turn baseline
-- (the initial direction message counts as turn one). Migration 059 raised
-- the ceiling to six; this reverts the stored constraint to three while
-- preserving every row that is already persisted.

CREATE TABLE planning_sessions_060 (
  id TEXT PRIMARY KEY,
  ...
  baseline_turn_count INTEGER NOT NULL DEFAULT 0 CHECK (baseline_turn_count BETWEEN 0 AND 3),
...
INSERT INTO planning_sessions_060(
...
ALTER TABLE planning_sessions_060 RENAME TO planning_sessions;

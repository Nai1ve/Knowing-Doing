...
CREATE TABLE IF NOT EXISTS learner_source_items (
  ...
  created_at TEXT NOT NULL, updated_at TEXT NOT NULL, UNIQUE(learner_id, provider, external_id)
);
...
CREATE TABLE IF NOT EXISTS source_sync_jobs (
  ...
  provider TEXT NOT NULL, status TEXT NOT NULL CHECK(status IN ('queued','running','succeeded','failed')),
  cursor TEXT, client_request_id TEXT, error_code TEXT, error_message TEXT, created_at TEXT NOT NULL, updated_at TEXT NOT NULL,

...
export const TEMPLATE_NODES: TemplateNode[] = [
  ...
  { key: 'mysql-slow-query', parentKey: 'data-performance', type: 'lab', title: 'MySQL 慢查询与索引', summary: '在知行 Gym 中从慢日志定位 SQL，并用 EXPLAIN 和索引验证判断。', points: ['慢日志', 'EXPLAIN', '联合索引'], standard: '完成一次 MySQL 慢查询排查，并用前后执行计划或结果验证结论。', minutes: 120, priority: 1, mode: 'lab' },
...
function nodeFrom(row: Row): RoadmapNode {
  return {
    id: str(row, 'id'), roadmapId: str(row, 'roadmap_id'), parentId: nullable(row, 'parent_id'), nodeKey: str(row, 'node_key'), nodeType: str(row, 'node_type') as RoadmapNode['nodeType'], title: str(row, 'title'), summary: str(row, 'summary'), capabilityKey: nullable(row, 'capability_key'), exerciseProfileKey: nullable(row, 'exercise_profile_key'),
    ...
    status: str(row, 'progress_status') as RoadmapNodeStatus, progressSource: str(row, 'progress_source'), completedAt: nullable(row, 'completed_at'), verifiedAt: nullable(row, 'verified_at'), progressRevision: num(row, 'progress_revision'), childCount: num(row, 'child_count'), evidence: parsed<Array<{ sourceType: string; sourceId: string; excerpt: string }>>(row.evidence_json, []),
  }
}
...
export class PlanningService {
  ...
  confirm(learnerId: string, id: string, revision: number, startUnitKey?: string): LearningPlan {
    ...
    const tx = this.db.transaction(() => {
      ...
      const insertUnit = this.db.prepare('INSERT INTO plan_units(id, plan_id, roadmap_node_id, position, title, objective, case_id, exercise_profile_key, status, availability, learning_mode, estimated_minutes, rationale, completed_at, source_refs_json) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NULL, \'[]\')')
        let startPlanUnitId: string | null = null
        selectedNodes.forEach((node, index) => { const planUnitId = randomUUID(); if (index === 0) startPlanUnitId = planUnitId; const mode = str(node, 'learning_mode') as PlanUnit['learningMode']; insertUnit.run(planUnitId, planId, str(node, 'id'), index + 1, str(node, 'title'), str(node, 'summary'), nullable(node, 'case_id'), nullable(node, 'exercise_profile_key'), index === 0 ? 'current' : 'upcoming', 'available', mode, num(node, 'estimated_minutes'), chosenOption?.rationale.join(' ') ?? '根据当前规划对话切出的一到两周学习单元。') })
  ...
  private currentLearning(plan: LearningPlan | null, roadmap: Roadmap | null): CurrentLearning | null {
    ...
    const node = unit.roadmapNodeId
      ? this.db.prepare('SELECT capability_key, exercise_profile_key, node_key, title, summary FROM roadmap_nodes WHERE id = ? AND roadmap_id = ?').get(unit.roadmapNodeId, plan.roadmapId ?? roadmap?.id ?? '') as Row | undefined
      : undefined
    const catalog = new PracticeEnvironmentCatalog(this.db)
    const capability = node?.capability_key ? catalog.byCapability(String(node.capability_key)) : null

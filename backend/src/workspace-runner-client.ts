...
export class FakeWorkspaceRunnerClient implements WorkspaceRunnerClient {
  private readonly runs = new Map<string, { files: Map<string, { content: string; revision: number }>; commands: string[]; ended: boolean }>()
  create(input: { templateKey: string; files: RunnerFileInput[]; commands: string[] }): Promise<RunnerCreateResult> {
    const runnerRunId = `fake-${this.runs.size + 1}`; this.runs.set(runnerRunId, { files: new Map(input.files.map((file) => [file.path, { content: file.content, revision: 1 }])), commands: input.commands, ended: false })

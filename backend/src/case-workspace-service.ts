...
import { parseCaseRequest, parseCaseSpec, isAllowedPythonCommand } from './case-schemas.js'
import type { CaseBuilderProvider } from './case-builder.js'
...
export class CaseWorkspaceService {
  ...
  createCaseRequest(learnerId: string, input: unknown): { case: LearningCase; job: CaseGenerationJob } {
    ...
    const node = this.db.prepare(`SELECT n.id, n.learning_mode, n.capability_key, r.learner_id, r.status
    ...
    const existingJob = this.db.prepare('SELECT * FROM case_generation_jobs WHERE learner_id = ? AND client_request_id = ?').get(learnerId, request.clientRequestId) as Row | undefined
    if (existingJob) {
      if (text(existingJob, 'input_fingerprint') !== fingerprint) throw new LabError('idempotency_conflict', 'clientRequestId 已对应另一份案例请求', 409)
      const existingCase = this.caseForLearner(learnerId, text(existingJob, 'learning_case_id'))
      return { case: existingCase, job: jobFrom(existingJob) }
    }
    const now = new Date().toISOString(); const existingCase = this.db.prepare('SELECT * FROM learning_cases WHERE learner_id = ? AND input_fingerprint = ?').get(learnerId, fingerprint) as Row | undefined
    const caseId = existingCase ? text(existingCase, 'id') : randomUUID()
    if (existingCase) {
      const existingJob = this.db.prepare('SELECT * FROM case_generation_jobs WHERE learner_id = ? AND learning_case_id = ? AND input_fingerprint = ? ORDER BY created_at DESC LIMIT 1').get(learnerId, caseId, fingerprint) as Row | undefined
      if (existingJob) return { case: this.caseForLearner(learnerId, caseId), job: jobFrom(existingJob) }
    }
    if (!existingCase) this.db.prepare(`INSERT INTO learning_cases(id, learner_id, roadmap_node_id, capability_key, template_key, input_kind, input_snapshot_json, input_fingerprint, provider, version, status, case_spec_json, created_at, updated_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'fixture', 1, 'generating', '{}', ?, ?)`).run(caseId, learnerId, request.roadmapNodeId, capability.capabilityKey, capability.templateKey, request.input.kind, JSON.stringify(inputSnapshot), fingerprint, now, now)
    const jobId = randomUUID(); this.db.prepare(`INSERT INTO case_generation_jobs(id, learner_id, learning_case_id, client_request_id, input_fingerprint, provider, status, attempt_count, created_at, updated_at)
      VALUES (?, ?, ?, ?, ?, 'fixture', 'queued', 0, ?, ?)`).run(jobId, learnerId, caseId, request.clientRequestId, fingerprint, now, now)
    const job = this.jobForLearner(learnerId, jobId); const savedCase = this.caseForLearner(learnerId, caseId)
    void this.processCaseJob(jobId).catch((error) => { console.error('[zhixing-case] generation_unhandled', { jobId, error: error instanceof Error ? error.message : String(error) }) })
  ...
  private async processCaseJob(jobId: string): Promise<void> {
    ...
    try {
      const spec = parseCaseSpec(await this.builder.build({ request: this.requestFromCase(item), source }))
      ...
      const code = error instanceof Error && error.message.startsWith('unsupported_command:') ? 'unsupported_command' : 'case_generation_failed'; const message = error instanceof Error ? error.message.slice(0, 500) : '案例生成失败'; const failedAt = new Date().toISOString()
      this.db.transaction(() => {
  ...
  async startPractice(learnerId: string, caseId: string): Promise<WorkspaceSummary> {
    return this.withLock(this.caseLocks, `${learnerId}:${caseId}`, async () => {
      ...
      try {
        ...
        this.repository.createArtifact({ learnerId, practiceRunId: practiceId, kind: 'external_text', sourceKind: 'workspace', verificationStatus: 'not_applicable', content: item.spec.scenario, metadata: { learningCaseId: caseId, provider: item.provider, fixtureVersion: 'python-order-summary-v1' } })
      } catch (error) {
  ...
  async saveFile(learnerId: string, workspaceId: string, path: string, content: string, expectedRevision: number): Promise<WorkspaceSummary> {
    return this.withLock(this.workspaceLocks, workspaceId, async () => {
      ...
      if (content.length > 262144) throw new LabError('file_too_large', '文件不能超过 256KB', 422)

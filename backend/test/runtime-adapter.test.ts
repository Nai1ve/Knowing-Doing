...
describe('DockerWorkspaceRuntimeAdapter', () => {
  it('only provisions a server-registered Docker environment', async () => {
    ...
    await expect(adapter.provision({ environmentKey: 'go-test-v1', environmentVersion: '1', files: [], commands: [] })).rejects.toMatchObject({ code: 'workspace_environment_unavailable' })
  })
})

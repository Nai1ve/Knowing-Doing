  ...
  it('reverts the baseline turn ceiling to three and preserves child rows', () => {
    ...
    try {
      ...
      try {
        ...
        upgraded.prepare('UPDATE planning_sessions SET baseline_turn_count = 3 WHERE id = ?').run('migration-session')
        expect(() => upgraded.prepare('UPDATE planning_sessions SET baseline_turn_count = 4 WHERE id = ?').run('migration-session')).toThrow()

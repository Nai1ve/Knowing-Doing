...
defineProps<{ milestones: Milestone[]; selectedNodeId?: string; goal?: string }>()
const emit = defineEmits<{ select: [nodeId: string] }>()
</script>

<template>
  <section class="tree-wrap" aria-label="整体学习路线">
    <button class="tree-root" type="button" @click="emit('select', 'goal')"><small>Target outcome · 总目标</small>{{ goal || '完成一项可回看的技术学习产出' }}</button>
...
.tree-branches { position: relative; display: grid; grid-template-columns: repeat(3, minmax(0, 1fr)); gap: 12px; margin-top: 27px; }

...
const canSend = computed(() => Boolean(draft.value.trim()) && !props.loading)
function submit() {
  ...
  <section class="tutor-agent" aria-labelledby="tutor-title">
    <header class="tutor-header">
      ...
      <span class="tutor-status" :class="{ working: loading }">
        <LoaderCircle v-if="loading" class="spin" :size="12" aria-hidden="true" />{{ loading ? '正在思考' : '随时可问' }}
      </span>
    </header>
    ...
    <form class="tutor-composer" @submit.prevent="submit">
      ...
      <textarea id="tutor-input" v-model="draft" rows="3" placeholder="描述你的判断、错误或下一步想尝试的操作…" :disabled="loading" @keydown="onKeydown" />
...
<style scoped>
.tutor-agent { display: grid; grid-template-rows: auto auto minmax(260px, 1fr) auto auto; min-height: 680px; overflow: hidden; border: 1px solid #c9ccc5; background: #fff; }
...
.tutor-transcript { min-height: 0; overflow: auto; padding: 17px; background: #fff; }.tutor-empty { display: grid; place-items: center; min-height: 260px; padding: 28px; color: #a45836; text-align: center; }.tutor-empty strong { margin-top: 10px; color: #4c5651; font: 400 18px var(--serif); }.tutor-empty p { max-width: 260px; margin: 7px 0 0; color: var(--muted); font-size: 10px; line-height: 1.6; }
...
.source-tray { max-height: 210px; overflow: auto; padding: 12px 17px; border-top: 1px solid var(--line); background: var(--paper-muted); }.source-heading { display: flex; align-items: center; justify-content: space-between; gap: 10px; color: #4c5651; font: 10px var(--mono); }.source-heading span { display: inline-flex; align-items: center; gap: 6px; }.source-heading small { color: var(--muted); font-size: 8px; }.source-list { display: grid; gap: 7px; margin-top: 8px; }.source-card { display: flex; justify-content: space-between; gap: 8px; padding: 8px 9px; border: 1px solid var(--line); background: #fff; }.source-card-copy { min-width: 0; }.source-card-copy strong { display: block; overflow: hidden; color: var(--ink); font-size: 10px; line-height: 1.4; text-overflow: ellipsis; white-space: nowrap; }.source-card-copy p { display: -webkit-box; margin: 4px 0; overflow: hidden; color: var(--muted); font-size: 9px; line-height: 1.4; -webkit-box-orient: vertical; -webkit-line-clamp: 2; }.source-card-copy small { color: #a45836; font: 8px var(--mono); }.source-card-actions { display: flex; gap: 2px; align-items: flex-start; flex: 0 0 auto; }.source-card-actions a, .source-card-actions button { display: grid; place-items: center; width: 24px; height: 24px; padding: 0; border: 0; background: transparent; color: var(--blue); text-decoration: none; }.source-card-actions a:hover, .source-card-actions button:hover, .source-card-actions button.pinned { background: var(--blue-soft); color: var(--blue-deep); }
...
@media (max-width: 600px) { .tutor-agent { min-height: 620px; }.tutor-header, .tutor-context, .tutor-transcript, .source-tray, .tutor-composer { padding-inline: 13px; } }
</style>

...
async function finish() { await gym.complete(reflection.value) }
</script>
    ...
    <template v-else-if="gym.session">
      ...
      <section class="activity-list" aria-label="Gym activities">
        <article v-for="activity in gym.session.activities" :key="activity.id" class="activity-card" :class="{ active: gym.currentActivity?.id === activity.id, done: ['correct', 'completed'].includes(state(activity)?.status ?? '') }">
          ...
          <div v-if="activity.type === 'runtime_practice'" class="runtime-card"><FlaskConical :size="18" /><div><strong>受限运行时</strong><p>运行时的输入、执行和完成信号由服务端控制；前端不持有答案或评分规则。</p></div><button v-if="gym.session.runtime?.status === 'not_started'" type="button" class="primary-button" :disabled="gym.saving" @click="gym.startRuntime">启动实践</button><button v-else type="button" class="secondary-button" :disabled="gym.loading" @click="gym.refreshSession"><RefreshCw :size="13" />刷新验证状态</button></div>
          <template v-else-if="activity.type !== 'reflection'">
            ...
            <textarea v-else v-model="answers[activity.id]" rows="3" :placeholder="activity.type === 'concept' ? '写下你的判断…' : '说明证据、判断与下一步动作…'" :aria-label="activity.title" />
          ...
          <template v-else><textarea v-model="answers[activity.id]" rows="4" placeholder="写下这次练习的迁移判断…" aria-label="反思" /><div class="activity-actions"><span>完成后会记录本次 Gym 的结果</span><button type="button" class="primary-button" :disabled="gym.completing || !isReady(activity)" @click="finish">{{ gym.completing ? '完成中…' : '完成 Gym' }}</button></div></template>

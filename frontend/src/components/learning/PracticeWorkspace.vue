...
import type { ProductPracticePin, ProductPracticeRun, ProductSnapshot, ProductTutorMessage, ProductTutorSource } from '@/types/product'
...
<template>
  <div class="workspace">
    <header class="workspace-bar">
      ...
      <div class="workspace-actions"><button v-if="!labRun" type="button" class="secondary-button" :disabled="practiceStarting" @click="emit('reopen')"><RotateCcw :size="12" aria-hidden="true" />{{ practiceStarting ? '续开中…' : '继续实践' }}</button><button v-else type="button" class="secondary-button" :disabled="labResetting || labEnding" @click="emit('reset')"><RotateCcw :size="12" aria-hidden="true" />{{ labResetting ? '重置中…' : '重置' }}</button><button v-if="labRun" type="button" class="danger-button" :disabled="labResetting || labEnding" @click="emit('end')"><Square :size="11" aria-hidden="true" />{{ labEnding ? '结束中…' : '结束' }}</button></div>
    </header>
    <div class="workspace-columns">
      <section class="workbench-column" aria-label="实验工作台">
        ...
        <SqlWorkbench :sql="labSql" :result="latestResult" :can-execute="Boolean(labRun && labReady && activeSessionName)" :executing="labExecuting" :session-name="activeSessionName" :result-artifact-id="resultArtifact?.id" :result-pinned="resultPinned" @update:sql="emit('update:sql', $event)" @execute="emit('execute')" @load-default="emit('load-default')" @load-create-index="emit('load-create-index')" @load-optimized="emit('load-optimized')" @pin="pinResult" />
      ...
      <aside class="tutor-column"><TutorAgent :messages="messages" :sources="sources" :source-status="sourceStatus" :loading="tutorLoading" :current-question="tutorQuestion" :current-gap="currentGap" :failure="tutorFailure" :pinned-ids="pinnedIds" @ask="emit('ask', $event)" @retry="emit('retry')" @pin="forwardPin" /></aside>
    </div>
  </div>
</template>

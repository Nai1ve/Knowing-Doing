...
defineProps<{ messages: AgentPlanningMessage[]; streamingAssistant: string; question: string; canGenerate: boolean; loading: boolean; generating: boolean }>()
...
<template>
  <section class="planning-chat" aria-labelledby="planning-chat-title">
    ...
    <div v-if="canGenerate" class="generation-ready"><Route :size="15" aria-hidden="true" /><div><strong>已经可以生成初版学习路线</strong><span>当前信息足够形成第一版方案，你也可以继续补充后再生成。</span></div></div><div class="next-question"><small>下一步可以聊聊</small><strong>{{ question || '你可以继续补充经历，也可以直接生成路线。' }}</strong></div>

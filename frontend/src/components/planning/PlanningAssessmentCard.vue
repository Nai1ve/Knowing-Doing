...
import { AlertTriangle, ChevronDown, ChevronRight, CircleCheck, CircleX, History, RotateCcw, Save, SkipForward } from 'lucide-vue-next'
import { computed, ref, watch } from 'vue'
import type { PlanningAssessment, PlanningAssessmentAnswer, PlanningAssessmentReview } from '@/types/product'
import { assessmentStatusLabel } from '@/utils/planning-flow'

const props = defineProps<{ assessment: PlanningAssessment | null; compact?: boolean; loading?: boolean; saving?: boolean; error?: string | null; review?: PlanningAssessmentReview | null; reviewLoading?: boolean }>()
...
const isSuperseded = computed(() => props.assessment?.status === 'superseded')
...
function toggleResult() { resultOpen.value = !resultOpen.value }
</script>

<template>
  <section class="assessment-card" aria-labelledby="assessment-title">
    <header class="assessment-header"><div><div class="eyebrow">Diagnostic assessment</div><h2 id="assessment-title">把会什么，变成可验证的起点</h2></div><span v-if="assessment" class="assessment-status">{{ assessmentStatusLabel(assessment.status) }}</span></header>
    ...
    <div v-else-if="safeError || assessment?.status === 'failed'" class="assessment-state error" role="alert"><AlertTriangle :size="16" aria-hidden="true" /><div><strong>评估没有完成</strong><p>{{ safeError ?? '评估准备没有完成，但已保存的对话和画像仍然保留，可以安全重试。' }}</p><small>不会丢失已保存的对话、简历和画像信息；重试只会重新准备当前测评。</small><button type="button" class="secondary-button" :disabled="loading || saving" @click="emit('retry')"><RotateCcw :size="13" aria-hidden="true" />{{ loading ? '正在恢复…' : '重试评估' }}</button></div></div>
    <div v-else-if="isSuperseded" class="assessment-state superseded" role="status"><History :size="16" aria-hidden="true" /><div><strong>评估已由新版本取代</strong><p>这份测评已经失效，新的评估进度会出现在上方，已保存的对话和画像仍然保留。</p></div></div>
    <template v-else-if="assessment && !isTerminal">
      ...
      <div v-if="assessment.recoveredFromFailure" class="assessment-recovered" role="status"><History :size="13" aria-hidden="true" />已使用稳定生成协议重新准备测评，之前的对话和画像都保留。</div>
      ...
      <div class="assessment-actions"><div class="assessment-navigation"><button type="button" class="text-button" :disabled="saving" @click="skipCurrent"><SkipForward :size="13" aria-hidden="true" />明确跳过</button><button type="button" class="text-button" :disabled="saving || currentIndex === 0" @click="currentIndex -= 1">上一题</button></div><div class="assessment-submission"><button v-if="currentIndex < assessment.questions.length - 1" type="button" class="secondary-button" :disabled="saving" @click="saveAndNext"><Save :size="13" aria-hidden="true" />{{ saving ? '保存中…' : '保存并继续' }}</button><button v-else type="button" class="primary-button" :disabled="saving" @click="complete"><CircleCheck :size="14" aria-hidden="true" />完成评估</button><button type="button" class="danger-button" :disabled="saving" @click="abandon"><CircleX :size="13" aria-hidden="true" />结束评估</button></div></div>
    </template>
    <template v-else-if="assessment && isTerminal">
      <div v-if="compact" class="assessment-compact"><CircleCheck :size="16" aria-hidden="true" /><div><strong>水平测评已完成</strong><span>结论已用于后续路线；现在请确认你的学习要求。</span></div><button type="button" class="text-button" @click="toggleResult">{{ resultOpen ? '收起测评报告' : '查看完整测评报告' }}</button></div>
      <template v-if="!compact || resultOpen"><div v-if="assessment.summary?.dimensions.length" class="assessment-result"><div class="result-intro"><CircleCheck :size="17" aria-hidden="true" /><div><strong>评估结果已保存</strong><p>以下是当前证据支持的维度判断，不合并成一个总分。</p></div></div><div class="dimension-list"><article v-for="dimension in assessment.summary.dimensions" :key="dimension.key"><div><strong>{{ dimension.label || dimension.key }}</strong><span>{{ dimension.level }} · 置信度 {{ Math.round(dimension.confidence * 100) }}%</span></div><p v-if="dimension.evidence.length">{{ dimension.evidence[0] }}</p><small>下一步验证：{{ dimension.nextValidation }}</small></article></div></div>

...
const scaleValues = computed(() => {
  ...
  if (!item || item.type !== 'scale') return []
  const min = item.min ?? 1; const max = item.max ?? 5
...
<template>
  <section class="assessment-card" aria-labelledby="assessment-title">
    ...
    <template v-else-if="assessment && !isTerminal">
      ...
      <article v-if="question" class="question"><small>{{ question.required === false ? '可跳过' : '请按当前理解回答' }}</small><h3>{{ question.prompt }}</h3><p v-if="question.description">{{ question.description }}</p>
    ...
    <template v-else-if="assessment && isTerminal">
      ...
      <div v-if="reviewOpen" class="review-list"><div v-if="reviewLoading">正在加载逐题复核…</div><article v-for="item in review?.items ?? []" :key="item.questionId"><strong>{{ item.question }}</strong><span>{{ item.skipped ? '已明确跳过' : item.answer === null ? '未作答' : Array.isArray(item.answer) ? item.answer.join('、') : item.answer }}</span></article></div>
    </template>
  </section>
</template>

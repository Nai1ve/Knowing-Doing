...
watch(() => props.brief, (value) => { draft.value = value ? { ...value, mustHave: [...value.mustHave], niceToHave: [...value.niceToHave], constraints: [...value.constraints], successCriteria: [...value.successCriteria] } : null }, { immediate: true })
function lines(value: string[]) { return value.join('\n') }
function updateList(key: 'mustHave' | 'niceToHave' | 'constraints' | 'successCriteria', event: Event) { if (draft.value) draft.value[key] = (event.target as HTMLTextAreaElement).value.split('\n').map((item) => item.trim()).filter(Boolean) }
function confirm() { if (draft.value) emit('confirm', draft.value) }
</script>

<template>
  <section v-if="draft" class="brief-card" aria-labelledby="requirements-title"><header><div><div class="eyebrow">Requirements brief</div><h2 id="requirements-title">把路线的交付边界说清楚</h2></div><span v-if="draft.status === 'confirmed'" class="confirmed">已确认</span></header><p class="brief-note">这是 Planner 根据对话和评估整理的工作约束。你可以直接修改，也可以在右侧对话中继续提出调整。</p><label class="field"><span>目标摘要</span><textarea v-model="draft.summary" rows="3" /></label><div class="brief-grid"><label class="field"><span>必须具备</span><textarea :value="lines(draft.mustHave)" rows="4" @input="updateList('mustHave', $event)" /></label><label class="field"><span>可选增强</span><textarea :value="lines(draft.niceToHave)" rows="4" @input="updateList('niceToHave', $event)" /></label><label class="field"><span>现实约束</span><textarea :value="lines(draft.constraints)" rows="4" @input="updateList('constraints', $event)" /></label><label class="field"><span>成功标准</span><textarea :value="lines(draft.successCriteria)" rows="4" @input="updateList('successCriteria', $event)" /></label></div><div class="brief-actions"><button type="button" class="text-button" @click="emit('revise')"><MessageCircle :size="14" aria-hidden="true" />在对话中继续修改</button><button type="button" class="primary-button" :disabled="saving" @click="confirm"><Check :size="14" aria-hidden="true" />{{ saving ? '确认中…' : '确认需求并准备路线' }}</button></div><p v-if="error" class="brief-error"><RotateCcw :size="13" aria-hidden="true" />{{ error }}</p></section>
</template>

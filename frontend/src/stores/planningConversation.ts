...
import { ref } from 'vue'
import { addPlanningTurn, adjustPlanning, createPlanningSession, getPlanningSession } from '@/api/planningService'
...
export const usePlanningConversationStore = defineStore('planningConversation', () => {
  ...
  async function start(goal?: string) { loading.value = true; error.value = null; try { session.value = await createPlanningSession(goal); return session.value } catch (cause) { error.value = cause instanceof Error ? cause.message : '规划对话创建失败'; throw cause } finally { loading.value = false } }
  ...
  return { session, draft, loading, error, start, load, answer, adjust }
})

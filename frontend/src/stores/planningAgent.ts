...
export const usePlanningAgentStore = defineStore('planningAgent', () => {
  ...
  async function updateRequirementBrief(brief: PlanningRequirementBrief) {
    if (!session.value) throw new Error('规划会话尚未加载'); requirementSaving.value = true; error.value = null
    try { await updatePlanningRequirementBrief(session.value.id, brief); applySession(await getAgentPlanningSession(session.value.id)); return session.value.requirementBrief }
    catch (cause) { error.value = cause instanceof Error ? cause.message : '需求修改失败，请重试'; throw cause }
    finally { requirementSaving.value = false }
  }

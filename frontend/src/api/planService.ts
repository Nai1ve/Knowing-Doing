...
import { createMockProductPlan, getMockProductPlan } from '@/data/mockProduct'
...
export async function getCurrentPlan(): Promise<ProductPlan | null> {
  if (useMockApi) return getMockProductPlan()
...
export async function createMysqlPerformancePlan(): Promise<ProductPlan> {
  if (useMockApi) return createMockProductPlan()

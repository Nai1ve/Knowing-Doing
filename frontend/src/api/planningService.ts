...
async function streamRequest(path: string, body: Record<string, unknown>, onEvent: (event: PlanningStreamEvent) => void): Promise<void> {
  const csrfToken = typeof localStorage === 'undefined' ? null : localStorage.getItem('zhixing.csrf-token')
...
export async function getPlanningResume(sessionId: string): Promise<ProductResumeAttachment | null> { return (await getAgentPlanningSession(sessionId)).resume }
...
export function sendPlanningRequirementsMessage(sessionId: string, message: string, clientRequestId = createClientId()): Promise<PlanningRequirementBrief> { return request<PlanningRequirementBrief>(`/product/planning-sessions/${sessionId}/requirements/messages`, { method: 'POST', body: JSON.stringify({ message, clientRequestId }) }) }
export function updatePlanningRequirementBrief(sessionId: string, brief: PlanningRequirementBrief, clientRequestId = createClientId()): Promise<PlanningRequirementBrief> { return request<PlanningRequirementBrief>(`/product/planning-sessions/${sessionId}/requirements/${brief.id}`, { method: 'PATCH', headers: { 'X-Client-Request-Id': clientRequestId }, body: JSON.stringify({ content: brief.content, clientRequestId }) }) }

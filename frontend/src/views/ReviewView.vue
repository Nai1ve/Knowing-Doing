...
<template>
  <div class="page review-page">
    <PageHeader eyebrow="05 · Review" title="每周复盘，调整下一段路。" description="复盘不只看完成了多少，更看你能否把原理迁移到新的查询、执行结果和错误现场。" :meta="['第 2 周', '计划 240 分钟', '当前节点：MySQL 性能实践']" />
    <AsyncState :loading="lessonStore.loading" :error="lessonStore.error"><template #default><section id="review-stats" class="review-stats"><SectionHeading title="本周学习数据" detail="Evidence over activity" /><div class="metric-grid"><div><BookMarked :size="15" aria-hidden="true" /><strong>{{ practiceCount }}</strong><span>实践素材</span></div><div><CircleAlert :size="15" aria-hidden="true" /><strong>{{ counts.error }}</strong><span>错误记录</span></div><div><TimerReset :size="15" aria-hidden="true" /><strong>240</strong><span>计划分钟</span></div><div><ArrowUpRight :size="15" aria-hidden="true" /><strong>42%</strong><span>计划完成度</span></div></div></section><section id="review-change" class="change-section"><SectionHeading title="掌握变化" detail="What changed" /><div class="change-list"><div><span class="change-icon up"><ArrowUpRight :size="14" aria-hidden="true" /></span><div><strong>开始解释执行证据</strong><p>你已经开始把查询现象、执行计划和结果集放在一起判断，而不是只修改一条语句。</p></div></div><div><span class="change-icon up"><ArrowUpRight :size="14" aria-hidden="true" /></span><div><strong>开始按证据排障</strong><p>遇到错误时会先记录 SQL 和执行输出，再让 Tutor Agent 帮你对照相关知识。</p></div></div><div><span class="change-icon down"><ArrowDownRight :size="14" aria-hidden="true" /></span><div><strong>还需要练习迁移</strong><p>下一步需要把相同的分析方法迁移到锁等待和深分页等新的性能问题。</p></div></div></div></section><section id="review-adjustment" class="adjustment"><div><div class="eyebrow">Suggested adjustment</div><h2>下一周把“解释”推进到“验证”。</h2><p>保留每天 35 分钟的线性学习，将一次练习替换为“故意制造一个错误，再用证据定位”的小实验。</p></div><div class="adjustment-note"><span>建议保留</span><strong>原理 → 示例 → 实践 → 记录</strong><small>学习顺序不变，只提高实践证据占比。</small></div></section></template></AsyncState>
  </div>
</template>

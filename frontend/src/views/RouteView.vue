...
const route = useRoute(); const router = useRouter(); const roadmap = useRoadmapStore(); const selectedId = ref(String(route.params.nodeId ?? '')); const openIds = ref<string[]>([]); const routeMismatch = ref(false); const navigationToken = ref(0); const adjustmentRequest = ref(''); const adjustmentPending = ref(false)
...
const selected = computed(() => {
  const node = selectedId.value ? roadmap.nodeById[selectedId.value] ?? null : roots.value[0] ?? null
  if (!node || !currentNodeId.value || node.id === currentNodeId.value || !['concept', 'lab', 'project'].includes(node.nodeType)) return node
  return { ...node, status: 'locked' as const }
})
const currentNodeId = computed(() => current.value?.currentLearning?.roadmapNodeId ?? current.value?.currentPlan?.units.find((item) => item.status === 'current')?.roadmapNodeId ?? null)
const currentPlanUnit = computed(() => current.value?.currentPlan?.units.find((unit) => unit.roadmapNodeId === selected.value?.id) ?? null)
const currentGymEntry = computed(() => {
...
const currentPracticeSetup = computed(() => {
  const learning = current.value?.currentLearning
  const entry = resolveLearningEntry(learning)
  return (entry === 'practice_setup' || entry === 'workspace_setup' || (entry === 'dynamic_gym' && !hasActivePractice(learning))) ? learning ?? null : null
})
const isCurrentLabNode = computed(() => Boolean(
  selected.value
  && currentGymEntry.value
  && selected.value.id === currentGymEntry.value.roadmapNodeId
  && selected.value.learningMode === 'lab',
))
...
async function requestAdjustment() { if (!current.value?.currentPlan || !adjustmentRequest.value.trim()) return; adjustmentPending.value = true; try { await roadmap.requestAdjustment(current.value.currentPlan.id, adjustmentRequest.value.trim()); adjustmentRequest.value = '' } finally { adjustmentPending.value = false } }
async function confirmAdjustment() { if (!roadmap.adjustment) return; await roadmap.confirmAdjustment(roadmap.adjustment.id); await syncRoute() }
</script>

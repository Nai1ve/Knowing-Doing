...
const filteredEvents = computed(() => notesStore.filter === 'all' ? lessonStore.events : lessonStore.events.filter((event) => event.type === notesStore.filter))
...
function createOutline() { if (planStore.plan) void notesStore.createOutline(planStore.plan.id) }

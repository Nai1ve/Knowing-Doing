...
import PlanningRoadmapGenerationCard from '@/components/planning/PlanningRoadmapGenerationCard.vue'
import PlanningRoadmapReadyCard from '@/components/planning/PlanningRoadmapReadyCard.vue'
...
const route = useRoute(); const router = useRouter(); const planning = usePlanningAgentStore(); const session = computed(() => planning.session); const uploadingResume = ref(false); const resumeUploadError = ref<string | null>(null); const requirementsTarget = ref<HTMLElement | null>(null); const editingConfirmedBrief = ref(false); const assessmentStage = computed(() => Boolean(session.value && isAssessmentStage(session.value.stage))); const requirementsStage = computed(() => Boolean(session.value && ['requirements', 'requirements_review', 'ready'].includes(session.value.stage))); const requirementsConversationStage = computed(() => Boolean(session.value && ['requirements', 'requirements_review'].includes(session.value.stage))); const showAssessment = computed(() => assessmentStage.value || Boolean(requirementsConversationStage.value && planning.assessment && ['completed', 'abandoned'].includes(planning.assessment.status))); const readyStage = computed(() => session.value?.stage === 'ready'); const generatingStage = computed(() => session.value?.stage === 'generating'); const showPlanningChat = computed(() => !assessmentStage.value && !generatingStage.value && (!readyStage.value || editingConfirmedBrief.value))
...
watch(() => session.value?.stage, (stage, previous) => { if (stage !== 'ready') editingConfirmedBrief.value = false; if ((previous && isAssessmentStage(previous) && (stage === 'requirements' || stage === 'requirements_review')) || (previous && ['requirements', 'requirements_review'].includes(previous) && stage === 'ready')) void focusRequirements() })
...
async function saveEditedBrief(brief: RequirementBrief) { await planning.updateRequirementBrief(brief); editingConfirmedBrief.value = false }
async function handleBrief(brief: RequirementBrief) { if (editingConfirmedBrief.value) await saveEditedBrief(brief); else await confirmBrief(brief) }
function reviseConfirmedBrief() { editingConfirmedBrief.value = true; void focusRequirements() }
</script>

<template>
  <div class="page planning-page"><AsyncState :loading="planning.streaming && !session" :error="planning.loadError"><template #default><PageHeader eyebrow="01 · Plan together" title="先聊清楚你想走向哪里。" description="把目标、经验和现实投入告诉 Planner。它会围绕关键未知继续追问，再整理成一张可以展开的能力路线图。" :meta="['基线 → 评估 → 需求 → 路线', '可恢复', '服务端判断是否可生成']" /><PlanningProgress v-if="session" :stage="session.stage" :progress="session.progress" /><div v-if="session" class="planning-layout"><main class="planning-main"><p v-if="planning.assessmentRecoveryNotice && showAssessment" class="recovery-notice" role="status">{{ planning.assessmentRecoveryNotice }}</p><PlanningAssessmentCard v-if="showAssessment" :assessment="planning.assessment" :compact="requirementsStage" :loading="planning.assessmentLoading" :saving="planning.assessmentSaving" :error="planning.assessmentError" :review="planning.assessmentReview" :review-loading="planning.reviewLoading" @retry="planning.retryAssessment" @save="saveAssessment" @complete="completeAssessment" @abandon="abandonAssessment" @review="planning.loadAssessmentReview" /><section v-if="requirementsConversationStage || editingConfirmedBrief" ref="requirementsTarget" class="requirements-flow" tabindex="-1" aria-label="需求确认"><div v-if="requirementsConversationStage" class="next-step-banner" role="status"><div class="eyebrow">Next step</div><strong>下一步：确认你的学习要求</strong><span>检查下方摘要；需要补充时直接继续对话，确认后才能生成路线。</span></div><div v-else class="next-step-banner" role="status"><div class="eyebrow">Editing requirements</div><strong>修改后需要重新确认</strong><span>保存修改会使当前确认失效，并回到需求复核步骤。</span></div><PlanningRequirementBrief v-if="session.requirementBrief" :brief="session.requirementBrief" :primary-label="editingConfirmedBrief ? '保存修改并重新确认' : undefined" :saving="planning.requirementSaving" :error="planning.error" @confirm="handleBrief" @revise="() => undefined" /></section><section v-else-if="readyStage && session.requirementBrief" ref="requirementsTarget" class="requirements-flow" tabindex="-1" aria-label="路线生成确认"><PlanningRoadmapReadyCard :brief="session.requirementBrief" :generating="planning.generating" @generate="generate" @revise="reviseConfirmedBrief" /></section><section v-else-if="generatingStage" ref="requirementsTarget" class="requirements-flow" tabindex="-1" aria-label="路线生成进度"><PlanningRoadmapGenerationCard :generation="planning.generation" :error="planning.error" :retrying="planning.generating" @retry="planning.retryGeneration" /></section><PlanningChat v-if="showPlanningChat" :messages="session.messages" :streaming-assistant="planning.streamingAssistant" :question="planning.question" :can-generate="session.readiness.canGenerateRoadmap" :readiness="session.readiness" :loading="planning.streaming" :generating="planning.generating" :generation="planning.generation" :error="planning.error" @send="planning.send" @generate="generate" @retry="planning.retry" @retry-generation="planning.retryGeneration" /></main><PlanningProfilePanel :session="session" :uploading="uploadingResume" :upload-error="resumeUploadError" :resume-notice="planning.resumeNotice" :resume-polling="planning.resumeParsing" :resume-poll-exhausted="planning.resumePollExhausted" :resume-poll-error="planning.resumePollError" @upload="upload" /></div></template></AsyncState></div>
</template>
